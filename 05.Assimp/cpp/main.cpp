...
void render_object()
{
  ...
  if (g_is_perspective)
  {
    cameras[cam_select_idx].set_mode(cameras[cam_select_idx].kPerspective);
    mat_proj = glm::perspective(glm::radians(cameras[cam_select_idx].fovy()), 1.0f, 0.001f, 100.0f);
  }
  else
  {
    cameras[cam_select_idx].set_mode(cameras[cam_select_idx].kOrtho);
